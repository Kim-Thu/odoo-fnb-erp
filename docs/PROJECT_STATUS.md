...
| Task đang active | T1010 — Product import template và field guide |
| Branch | `docs/T1010-product-import-guide-20260811-1730` |
| PR | Chưa mở — sẽ mở sau commit |
| PR state | Chưa có |
| CI gần nhất | T1009/PR #22 green và đã merged |
...
| Phase 1 — Master data / shared configuration | 🔵 Đang triển khai | Product master T1001–T1009 done; T1010 đang review; sau đó UoM, partner master, warehouse foundation |
...
Sau khi T1010 merge, tiếp tục Phase 1:

1. `T1101` — Định nghĩa cấu hình UoM theo hướng Standard First.
2. `T1102` — Test UoM conversion hợp lệ trong cùng category.
3. `T1103` — Test UoM conversion sai giữa các category.
4. `T1104` — Định nghĩa customer/vendor master data.
5. `T1105` — Test demo vendor/customer master.
6. `T1106` — Partner import template và field guide.
7. `T1201` — Định nghĩa cấu trúc Raw Materials / Production / Finished Goods.
8. `T1202`–`T1203` — Demo warehouse/location và test internal transfer.
...
- [x] T1009/PR #22 đã merge và CI xanh.
- [x] Chỉ có một task nghiệp vụ active: T1010.
- [x] Dependency T1004 của T1010 đã `done`.
- [x] T1010 không thay đổi mapping requirement; traceability matrix chưa cần sửa.
