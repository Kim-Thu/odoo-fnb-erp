...
| Task đang active | T1101 — Xác định cấu hình UoM theo hướng Standard First |
| Branch | `docs/T1101-uom-standard-first-20260811-1742` |
| PR | Chưa mở — sẽ mở sau commit |
| PR state | Chưa có |
| CI gần nhất | T1010/PR #23 green và đã merged |
...
| Phase 1 — Master data / shared configuration | 🔵 Đang triển khai | Product master T1001–T1010 done; T1101 đang review; tiếp theo T1102–T1106 và T1201–T1203 |
...
Sau khi T1101 merge, tiếp tục Phase 1:

1. `T1102` — Test UoM conversion hợp lệ trong cùng category.
2. `T1103` — Test UoM conversion sai giữa các category.
3. `T1104` — Định nghĩa customer/vendor master data.
4. `T1105` — Test demo vendor/customer master.
5. `T1106` — Partner import template và field guide.
6. `T1201` — Định nghĩa cấu trúc Raw Materials / Production / Finished Goods.
7. `T1202`–`T1203` — Demo warehouse/location và test internal transfer.
...
- [x] T1010/PR #23 đã merge và CI xanh.
- [x] Chỉ có một task nghiệp vụ active: T1101.
- [x] Dependency T0016 của T1101 đã `done`.
- [x] T1101 vẫn nằm trong mapping BR-01 / FR-MD-02; traceability matrix chưa cần đổi.
